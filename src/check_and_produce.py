...
Sik araliklarla calisir (cron, or. her 5-10 dakikada bir). DORT asamali surec:
  ...
  asama 3 "reviewing"          -> Kullanici "Onayla" dedi mi, "Tekrar uret" mi
                         dedi? Onayladiysa YouTube'a yukler (YouTube'un kendi
                         PUBLISH_TIME_ISTANBUL zamanlamasiyla) -> durum "done".
...
def run() -> None:
    ...
    # de yoktu. Otomatik saat tetiklemesi gunde sadece BIR kez calisir.
...
def _produce_and_upload(pending: dict, chosen: dict) -> None:
    """Onaylanan videoyu YENIDEN URETIP YouTube'a yukler. Not: Telegram Bot
    API'nin getFile ile dosya INDIRME siniri 20MB (gonderme siniri 50MB'dir) --
    shorts videolarimiz genelde bunu asiyor, bu yuzden Telegram'dan geri
    indirmek yerine ayni icerikle videoyu YENIDEN URETIP oyle yukluyoruz."""
    try:
        with tempfile.TemporaryDirectory() as tmp:
            video_path = produce_video(chosen, Path(tmp))
